...
#             data_all = data_all - cur
    ...
    def retrieve_cpu(self, x, index, train=True, chunk_size=512):
        ...
        with torch.no_grad():
            for batch_x, batch_y, batch_x_mark, batch_y_mark, index in tqdm(rt_loader, desc="Retrieval Progress"):
                if 'traffic' in data.root_path or 'electricity' in data.root_path:
                    pred_from_retrieval = self.retrieve_cpu(batch_x.float().to(data.device), index, train=train, chunk_size=256)
                else:
