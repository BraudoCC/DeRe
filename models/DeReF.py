...
class Model(nn.Module):
    def __init__(self, configs,
        ...
        if not self.gym_series_sampling:
            if 'short_term' in self.task_name:
                _dataset_name = 'm4'
            else:
                _dataset_name = configs.data_path.split('.')[0]
            print(self.task_name)
            self.q_mat_dir, self.q_out_mat_dir = get_q_mat_path(self.seq_len, self.pred_len, _dataset_name, self.configs)
    ...
    def prepare_dataset(self, train_data, valid_data, test_data):
        ...
        self.retrieval_dict['train'] = train_rt.detach().cpu()
        self.retrieval_dict['valid'] = valid_rt.detach().cpu()
        self.retrieval_dict['test'] = test_rt.detach().cpu()
