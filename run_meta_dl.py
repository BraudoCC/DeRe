...
def _train_fold_worker(args_dict):
    """
    工作函数：训练单个 fold。
    用于并行训练时在子进程/线程中运行。
    ...
    train_data_np = args_dict['train_data']
    val_data_np = args_dict['val_data']
    testset_components_np = args_dict['testset_components']
    testset_meta_np = args_dict['testset_meta']
    y_trues = args_dict['y_trues']
    y_trues_mae = args_dict['y_trues_mae']
    name_components = args_dict['name_components']
    gpu_id = args_dict['gpu_id']
    hyperparams = args_dict['hyperparams']
    meta_model_type = args_dict['meta_model_type']
    seed = args_dict['seed']
    k_folds = args_dict['k_folds']

    current_seed = seed + fold_idx
    ...
    print(f"[Fold {fold_idx + 1}/{k_folds}] Training on {device}, val_dataset={val_dataset}")
    
    # 转换数据为 tensor
    train_components = torch.from_numpy(train_data_np['components']).long().to(device)
    train_meta = torch.from_numpy(train_data_np['meta_features']).float().to(device)
    train_targets = torch.from_numpy(train_data_np['targets']).float().to(device)
    
    val_components = torch.from_numpy(val_data_np['components']).long().to(device)
    val_meta = torch.from_numpy(val_data_np['meta_features']).float().to(device)
    val_targets = torch.from_numpy(val_data_np['targets']).float().to(device)
    
    testset_components = torch.from_numpy(testset_components_np).long().to(device)
    testset_meta = torch.from_numpy(testset_meta_np).float().to(device)
    ...
    trainloader = DataLoader(trainset, batch_size=batch_size, shuffle=True, drop_last=True, generator=g)
    valloader = DataLoader(valset, batch_size=batch_size, shuffle=False, drop_last=False)
    
    # 创建模型
    n_col = hyperparams['n_col']
    d_model = hyperparams['d_model']
    meta_feature_dim = hyperparams['meta_feature_dim']
    dropout = hyperparams['dropout']
    nhead = hyperparams['nhead']
    n_layers = hyperparams['n_layers']
    
    is_icl_model = 'icl' in meta_model_type
    is_simple_icl = 'icl-simple' in meta_model_type
    
    # ICL-specific hyperparameters
    k = hyperparams.get('k', 0.0)
    temporal = hyperparams.get('temporal', 1.0)
    ...
    elif meta_model_type == 'icl-tabpfn':
        model = MetaICLTabPFN(
            n_col=n_col,
    ...
    optimizer = model.configure_optimizers(weight_decay=weight_decay, learning_rate=lr, device_type='cuda' if torch.cuda.is_available() else 'cpu')
    
    # 损失函数 (Pearson correlation loss)
    def loss_pearson(y_pred, y_true):
    ...
    es_tol = hyperparams['es_tol']
    icl_shuffle = hyperparams.get('icl_shuffle', False)
    icl_batch = hyperparams.get('icl_batch', False)
    ...
    epoch_results = {'train_loss': [], 'val_loss': []}
    
    if meta_model_type == 'icl-tabpfn':
        # Skip training loop for pre-trained TabPFN
        best_model_state = {}
        best_epoch = 0
        best_val_loss = 0.0 # Not computed
        print("Skipping training loop for icl-tabpfn")
    else: 
        for epoch in range(epochs):
            ...
            if is_icl_model:
                if icl_batch:
                    ...
                    for batch in trainloader:
                        ...
                        optimizer.zero_grad()
                        _, y_pred = model(train_data=(component, meta_feature), test_data=None)
                        loss = loss_pearson(y_pred.squeeze(), y_true)
                        loss.backward()
                        optimizer.step()
                    ...
                    optimizer.zero_grad()
                    _, y_pred = model(train_data=(curr_c, curr_m), test_data=None)
                    loss = loss_pearson(y_pred.squeeze(), curr_t)
                    loss.backward()
                    optimizer.step()
                    train_losses.append(loss.item())
            else:
                for batch in trainloader:
                    component, meta_feature, y_true = batch
                    optimizer.zero_grad()
                    _, y_pred = model(component, meta_feature)  # meta_predictor 接收两个参数
                    loss = loss_pearson(y_pred.squeeze(), y_true)
                    loss.backward()
                    optimizer.step()
                    train_losses.append(loss.item())
            ...
            with torch.no_grad():
                if is_icl_model:
                    _, y_pred = model(train_data=(train_components, train_meta), test_data=(val_components, val_meta))
                    loss = loss_pearson(y_pred.squeeze(), val_targets)
    ...
    with torch.no_grad():
        if is_icl_model:
            # MetaICL: 上下文学习，需要传入训练数据作为上下文
            # train_data = (train_components, train_meta), test_data = (test_components, test_meta)
            kwargs = {}
            if meta_model_type == 'icl-tabpfn':
            ...
            _, y_pred = model(
                ...
                test_data=(testset_components, testset_meta),
                **kwargs
            )
            y_preds = y_pred.squeeze().cpu().numpy()
        else:
            # meta_predictor: 直接对测试集逐 batch 预测
            testset = TensorDataset(testset_components, testset_meta)
            testloader = DataLoader(testset, batch_size=batch_size, shuffle=False)
            for batch in testloader:
                component, meta_feature = batch
                _, y_pred = model(component, meta_feature)
                y_preds.append(y_pred.squeeze().cpu())
            y_preds = torch.cat(y_preds).numpy()
    
    # 计算 top1 表现
    has_valid_targets = not np.isnan(y_trues).all()
    if has_valid_targets:
        fold_top1_idx = np.argmin(y_preds)
        top1_perf = y_trues[fold_top1_idx]
        top1_perf_mae = y_trues_mae[fold_top1_idx]
        top1_name = name_components[fold_top1_idx]
    else:
        top1_perf = np.nan
        top1_perf_mae = np.nan
        top1_name = name_components[np.argmin(y_preds)]
    
    print(f"[Fold {fold_idx + 1}/{k_folds}] Done: best_epoch={best_epoch}, val_loss={best_val_loss:.4f}, top1_mse={top1_perf:.6f}")
    ...
    return {
        ...
        'model_state': best_model_state_cpu,
...
class Meta():
    ...
    def components_processing(self, task_name, datasets, test_dataset, meta_feature_type,
        ...
        logger.info(f"Training datasets available for K-fold: {list(self.dataset_data.keys())}")
    ...
    def meta_fit_baseline_nn(self):
        """
        Baseline: Nearest Neighbor 方法（不需要训练，不需要 K-fold）
    ...
    def _get_fold_result_path(self, val_dataset):
        """获取单个 fold 结果的保存路径"""
        max_size_suffix = f'-maxsize_{self.max_size}' if self.max_size else ''
        expand_suffix = f'-expand_{self.expand_testset}' if hasattr(self, 'expand_testset') else ''
        clip_suffix = f'-clip_{self.clip_timestamps}' if hasattr(self, 'clip_timestamps') else ''
        icl_suffix = f'-ishuf_{self.icl_shuffle}-ibatch_{self.icl_batch}' if hasattr(self, 'icl_shuffle') else ''
        # ICL-specific suffix to avoid overwriting results with different k/temporal
        ...
        if "icl" in getattr(self, "meta_model_type", ""):
            ...
            ktemp_suffix = ""
        hp_suffix = f'-lr_{self.lr}-dm_{self.d_model}-nl_{self.n_layers}-nh_{self.nhead}-do_{self.dropout}{icl_suffix}{ktemp_suffix}'
        
        fold_result_dir = (
            f'./meta/results/results_{self.meta_feature_type}/fold_results/'
            f'{self.suffix + "_" if self.suffix else ""}{self.test_dataset}-model_{self.meta_model_type}'
            f'-component_balance_{self.arg_component_balance}'
            f'-add_transformer_{self.arg_add_transformer}'
            f'-add_LLM_{self.arg_add_LLM}'
            ...
            f'_{self.pred_len_1}_{self.pred_len_2}'
        )
        os.makedirs(fold_result_dir, exist_ok=True)
        return f"{fold_result_dir}/fold_{val_dataset}.npz"

    def save_single_fold_result(self, val_dataset, fold_result, y_preds, y_trues, y_trues_mae):
        """
        保存单个 fold 的训练结果，用于并行训练后的集成。
        
        Args:
            val_dataset: 验证数据集名称（也是 fold 名称）
            fold_result: 该 fold 的训练结果字典
            y_preds: 该 fold 模型在测试集上的预测
            y_trues: 测试集真实值 (MSE)
            y_trues_mae: 测试集真实值 (MAE)
        """
        fold_path = self._get_fold_result_path(val_dataset)
        ...
        np.savez(
            ...
            top1_perf=fold_result.get('top1_perf', np.nan),
            top1_perf_mae=fold_result.get('top1_perf_mae', np.nan),
            top1_name=fold_result.get('top1_name', ''),
            ...
            # 元信息
            name_components=np.array(self.name_components, dtype=object),
            test_dataset=self.test_dataset,
            pred_len_1=self.pred_len_1,
            pred_len_2=self.pred_len_2,
        )
        logger.info(f"Single fold result saved to: {fold_path}")

    def load_single_fold_result(self, val_dataset):
        """加载单个 fold 的结果"""
        fold_path = self._get_fold_result_path(val_dataset)
        ...
        return {
            ...
            'model_state': data['model_state_dict'].item() if data['model_state_dict'].ndim == 0 else data['model_state_dict'],
            'name_components': list(data['name_components']),
        }

    def _save_ensemble_results(self, fold_results, fold_predictions, fold_ranks, 
                                ensemble_ranks, ensemble_preds_raw, y_trues, y_trues_mae,
                                available_datasets, k_folds, top1_perf, top1_perf_mae, top_name):
        """
        保存集成结果（供 ensemble_from_saved_folds 使用）。
        """
        ...
        save_path = (
            ...
            f'{max_size_suffix}{expand_suffix}{clip_suffix}{hp_suffix}'
            f'_{self.pred_len_1}_{self.pred_len_2}_kfold.npz'
        )
        
        has_valid_targets = not np.isnan(y_trues).all()
        ensemble_preds = ensemble_ranks
        
        # 计算排名统计
        if has_valid_targets:
            pred_ranks = np.argsort(np.argsort(ensemble_preds)) + 1
            true_ranks = np.argsort(np.argsort(y_trues)) + 1
            topk = 5
            true_topk_indices = np.argsort(y_trues)[:topk]
            pred_topk_indices = np.argsort(ensemble_preds)[:topk]
        ...
        topk_names = [self.name_components[i] for i in np.argsort(ensemble_preds)[:5]]
        
        # 准备每折的详细结果
        fold_details = {}
        for val_dataset, result in fold_results.items():
            fold_details[val_dataset] = {
                'best_epoch': result['best_epoch'],
                'best_val_loss': result['best_val_loss'],
                'top1_perf': result.get('top1_perf', np.nan),
                'top1_perf_mae': result.get('top1_perf_mae', np.nan),
                'top1_name': result.get('top1_name', ''),
            }
        ...
        np.savez_compressed(
            ...
            # K折结果
            k_folds=k_folds,
            fold_val_datasets=available_datasets,
            ...
            pred_len_1=self.pred_len_1,
            pred_len_2=self.pred_len_2,
            ensemble_mode='from_saved_folds',
        )
        
        logger.info(f"Ensemble results saved to: {save_path}")
        
        # =================== 保存 Attention Weights（如果启用）===================
        if self.save_attention and hasattr(self, 'meta_model_type') and 'icl' in self.meta_model_type:
            # 收集所有 fold 的 attention 信息
            attention_data = {}
            for val_dataset, result in fold_results.items():
                if 'attention_info' in result and result['attention_info'] is not None:
                    attn_info = result['attention_info']
                    # 保存所有四个子矩阵
                    fold_attn = {
                        'N_train': attn_info['N_train'],
                        'N_test': attn_info['N_test'],
                    }
                    for key in ['train_to_train', 'train_to_test', 'test_to_train', 'test_to_test']:
                        if key in attn_info and attn_info[key] is not None:
                            fold_attn[key] = attn_info[key]
                    if any(k in fold_attn for k in ['train_to_train', 'train_to_test', 'test_to_train', 'test_to_test']):
                        attention_data[val_dataset] = fold_attn
            ...
            if attention_data:
                ...
                np.savez_compressed(
                    ...
                    fold_val_datasets=available_datasets,
                    # attention_data 格式：
                    # attention_data[fold_name] = {
                    #   'N_train', 'N_test',
                    #   'train_to_train': (num_layers, N_train, N_train),
                    #   'train_to_test': (num_layers, N_train, N_test),
                    #   'test_to_train': (num_layers, N_test, N_train),
                    #   'test_to_test': (num_layers, N_test, N_test),
                    # }
    ...
    def ensemble_from_saved_folds(self):
        """
        从已保存的 fold 结果进行集成。
        用于所有并行 fold 训练完成后的最终集成步骤。
        
        Returns:
            dict: 集成结果，与 meta_fit_kfold 返回格式相同
        """
        available_datasets = [d for d in self.datasets_train if d in self.dataset_data]
        k_folds = len(available_datasets)
        
        logger.info(f"\n{'='*60}")
        logger.info(f"Ensemble from {k_folds} saved fold results")
        logger.info(f"Expected folds: {available_datasets}")
        logger.info(f"{'='*60}\n")
        ...
        for val_dataset in available_datasets:
            fold_data = self.load_single_fold_result(val_dataset)
            if fold_data is None:
                missing_folds.append(val_dataset)
                logger.warning(f"Missing fold result for: {val_dataset}")
            else:
                fold_results[val_dataset] = {
                    'best_epoch': fold_data['best_epoch'],
                    'best_val_loss': fold_data['best_val_loss'],
                    'top1_perf': fold_data['top1_perf'],
                    'top1_perf_mae': fold_data['top1_perf_mae'],
                    'top1_name': fold_data['top1_name'],
                    'model_state': fold_data['model_state'],
                }
                fold_predictions.append(fold_data['y_preds'])
                # 使用第一个 fold 的数据初始化共享变量
                if len(fold_predictions) == 1:
                    y_trues = fold_data['y_trues']
                    y_trues_mae = fold_data['y_trues_mae']
                    self.name_components = fold_data['name_components']
                logger.info(f"Loaded fold {val_dataset}: best_epoch={fold_data['best_epoch']}, "
                           f"val_loss={fold_data['best_val_loss']:.4f}, top1_perf={fold_data['top1_perf']:.6f}")
        
        if missing_folds:
            logger.error(f"Missing fold results for: {missing_folds}")
            logger.error(f"Please run the missing folds first with --fold_idx parameter")
            raise ValueError(f"Missing fold results: {missing_folds}")
        
        # 如果需要保存 attention，在此处获取
        is_icl_model = hasattr(self, 'meta_model_type') and 'icl' in self.meta_model_type
        should_save_attention = self.save_attention and is_icl_model
        
        if should_save_attention:
            logger.info("Collecting attention scores from loaded fold models...")
            for val_dataset, result in fold_results.items():
                if 'model_state' in result and result['model_state'] is not None:
                    # 准备该 fold 的训练数据
                    train_data, _ = self.prepare_fold_data(val_dataset)
                    # 初始化模型并获取 attention
                    self.meta_init()
                    _, attention_info = self.meta_predict_with_model(
                        result['model_state'], train_data, return_attention=True
                    )
                    result['attention_info'] = attention_info
                    logger.info(f"  Fold {val_dataset}: attention collected")
        
        # 以下是 meta_fit_kfold 中的集成逻辑
        has_valid_targets = not np.isnan(y_trues).all()
        
        # 将每折的预测值转换为rank
        fold_ranks = []
        for y_preds in fold_predictions:
            ranks = np.argsort(np.argsort(y_preds)) + 1
            fold_ranks.append(ranks)
        
        fold_ranks_array = np.stack(fold_ranks)
        ensemble_preds_raw = np.mean(np.stack(fold_predictions), axis=0)
        
        # 支持多种rank聚合方式
        rank_agg_method = getattr(self, 'rank_aggregation_method', 'median')
        
        if rank_agg_method == 'mean':
            ensemble_ranks = np.mean(fold_ranks_array, axis=0)
            logger.info(f"Ensemble method: Rank mean")
        elif rank_agg_method == 'median':
            ensemble_ranks = np.median(fold_ranks_array, axis=0)
            logger.info(f"Ensemble method: Rank median (robust to outliers)")
        elif rank_agg_method == 'trimmed_mean':
            if k_folds >= 3:
                sorted_ranks = np.sort(fold_ranks_array, axis=0)
                trimmed_ranks = sorted_ranks[1:-1, :]
                ensemble_ranks = np.mean(trimmed_ranks, axis=0)
            else:
                ensemble_ranks = np.mean(fold_ranks_array, axis=0)
            logger.info(f"Ensemble method: Trimmed mean (exclude min/max)")
        elif rank_agg_method == 'voting':
            vote_topk = 10
            n_samples = fold_ranks_array.shape[1]
            vote_counts = np.zeros(n_samples)
            for fold_rank in fold_ranks_array:
                vote_counts[fold_rank <= vote_topk] += 1
            rank_median = np.median(fold_ranks_array, axis=0)
            ensemble_ranks = -vote_counts * n_samples + rank_median
            logger.info(f"Ensemble method: Voting (top-{vote_topk} counts) + median tiebreaker")
        else:
            ensemble_ranks = np.median(fold_ranks_array, axis=0)
            logger.info(f"Ensemble method: Rank median (default)")
        ...
        top1_perf = y_trues[top1_idx] if has_valid_targets else np.nan
        top1_perf_mae = y_trues_mae[top1_idx] if has_valid_targets else np.nan
        top_name = self.name_components[top1_idx]
        topk_names = [self.name_components[i] for i in np.argsort(ensemble_preds)[:5]]
        
        logger.info(f"\n{'='*60}")
        logger.info(f"Ensemble Results (K={k_folds} models)")
        ...
        # 保存集成结果（与 meta_fit_kfold 相同的格式）
        self._save_ensemble_results(
            fold_results=fold_results,
            fold_predictions=fold_predictions,
            fold_ranks=fold_ranks,
            ensemble_ranks=ensemble_ranks,
            ensemble_preds_raw=ensemble_preds_raw,
            y_trues=y_trues,
            y_trues_mae=y_trues_mae,
    ...
    def evaluate_with_trained_models(self, trained_fold_results):
        """
        使用已训练好的模型对当前测试集进行评估并保存结果。
        用于 arg_all_periods=True 时，训练一次，然后对不同 pred_len 的测试集分别评估。
        
        Args:
            trained_fold_results: 之前训练返回的结果字典，包含每个 fold 的模型状态
        ...
        logger.info(f"\n{'='*60}")
        logger.info(f"Evaluating with {k_folds} trained models on test set (pred_len={self.pred_len_1}/{self.pred_len_2})")
        logger.info(f"Test dataset: {self.test_dataset}")
        logger.info(f"{'='*60}\n")
        ...
        for val_dataset in available_datasets:
            if val_dataset not in trained_fold_results:
                logger.warning(f"No trained model for fold {val_dataset}, skipping...")
                continue
            
            result = trained_fold_results[val_dataset]
            model_state = result.get('model_state')
            if model_state is None:
                logger.warning(f"No model_state for fold {val_dataset}, skipping...")
                continue
            
            # 准备该 fold 的训练数据（用于 ICL 模型的上下文）
            train_data, _ = self.prepare_fold_data(val_dataset)
            
            # 初始化模型并加载权重进行预测
            self.meta_init()
            y_preds = self.meta_predict_with_model(model_state, train_data)
            fold_predictions.append(y_preds)
            
            # 计算该折模型的 top1 表现
            if has_valid_targets:
                fold_top1_idx = np.argmin(y_preds)
                fold_top1_perf = y_trues[fold_top1_idx]
                fold_top1_perf_mae = y_trues_mae[fold_top1_idx]
                fold_top1_name = self.name_components[fold_top1_idx]
            else:
                fold_top1_perf = np.nan
                fold_top1_perf_mae = np.nan
                fold_top1_name = self.name_components[np.argmin(y_preds)]
            
            fold_results[val_dataset] = {
                'best_epoch': result.get('best_epoch', 0),
                'best_val_loss': result.get('best_val_loss', 0),
                'top1_perf': fold_top1_perf,
                'top1_perf_mae': fold_top1_perf_mae,
                'top1_name': fold_top1_name,
                'model_state': model_state,
            }
            
            logger.info(f"Fold {val_dataset}: Top1 MSE={fold_top1_perf:.6f}, Top1 MAE={fold_top1_perf_mae:.6f}")
        
        if len(fold_predictions) == 0:
            logger.error("No predictions were generated. Check if models were trained properly.")
            return None
        ...
        if rank_agg_method == 'mean':
            ...
            ensemble_ranks = np.median(fold_ranks_array, axis=0)
        elif rank_agg_method == 'trimmed_mean' and k_folds >= 3:
            sorted_ranks = np.sort(fold_ranks_array, axis=0)
            trimmed_ranks = sorted_ranks[1:-1, :]
            ensemble_ranks = np.mean(trimmed_ranks, axis=0)
        else:
        ...
        # 计算最终性能
        top1_idx = np.argmin(ensemble_ranks)
        top1_perf = y_trues[top1_idx] if has_valid_targets else np.nan
        top1_perf_mae = y_trues_mae[top1_idx] if has_valid_targets else np.nan
        top_name = self.name_components[top1_idx]
        topk_names = [self.name_components[i] for i in np.argsort(ensemble_ranks)[:5]]
        
        logger.info(f"\n{'='*60}")
        logger.info(f"Evaluation Results for pred_len={self.pred_len_1}/{self.pred_len_2}")
        logger.info(f"Top 1 Combination: {top_name}")
        if has_valid_targets:
            logger.info(f"Top 1 Performance (MSE): {top1_perf:.6f}")
            logger.info(f"Top 1 Performance (MAE): {top1_perf_mae:.6f}")
        ...
        self._save_ensemble_results(
            ...
            y_trues=y_trues,
            y_trues_mae=y_trues_mae,
            available_datasets=list(fold_results.keys()),
    ...
    def meta_fit_single_fold_only(self, fold_idx):
        """
        只训练指定的单个 fold 并保存结果。
        用于并行训练场景。
        
        Args:
            fold_idx: fold 索引（0-based）
        """
        available_datasets = [d for d in self.datasets_train if d in self.dataset_data]
        k_folds = len(available_datasets)
        
        if fold_idx < 0 or fold_idx >= k_folds:
            raise ValueError(f"fold_idx {fold_idx} out of range [0, {k_folds})")
        
        val_dataset = available_datasets[fold_idx]
        
        logger.info(f"\n{'='*60}")
        logger.info(f"Training single fold {fold_idx + 1}/{k_folds}: Validation on {val_dataset}")
        logger.info(f"Test dataset: {self.test_dataset}")
        logger.info(f"{'='*60}\n")
        ...
        # 准备该折的数据
        train_data, val_data = self.prepare_fold_data(val_dataset)
        logger.info(f"Train samples: {len(train_data['targets'])}, Val samples: {len(val_data['targets'])}")
        
        # 初始化新模型
        self.meta_init()
        
        # 训练该折
        fold_result = self.meta_fit_single_fold(train_data, val_data, val_dataset)
        
        # 在测试集上预测（可选获取 attention weights）
        is_icl_model = hasattr(self, 'meta_model_type') and 'icl' in self.meta_model_type
        should_save_attention = self.save_attention and is_icl_model
        
        if should_save_attention:
            y_preds, attention_info = self.meta_predict_with_model(
                fold_result['model_state'], train_data, return_attention=True
            )
        else:
            y_preds = self.meta_predict_with_model(fold_result['model_state'], train_data)
            attention_info = None
        ...
        if has_valid_targets:
            ...
            fold_top1_perf_mae = y_trues_mae[fold_top1_idx]
            fold_top1_name = self.name_components[fold_top1_idx]
            fold_result['top1_perf'] = fold_top1_perf
            fold_result['top1_perf_mae'] = fold_top1_perf_mae
            fold_result['top1_name'] = fold_top1_name
            fold_result['attention_info'] = attention_info  # 保存 attention 信息
            logger.info(f"Fold {val_dataset}: Top1 MSE={fold_top1_perf:.6f}, Top1 MAE={fold_top1_perf_mae:.6f}")
        else:
            fold_result['top1_perf'] = np.nan
            fold_result['top1_perf_mae'] = np.nan
            fold_result['top1_name'] = self.name_components[np.argmin(y_preds)]
        
        logger.info(f"Fold {val_dataset}: Best epoch={fold_result['best_epoch']}, Best val loss={fold_result['best_val_loss']:.4f}")
        
        # 保存该 fold 的结果
        self.save_single_fold_result(val_dataset, fold_result, y_preds, y_trues, y_trues_mae)
    ...
    def _train_folds_parallel(self, available_datasets, y_trues, y_trues_mae, max_workers=None, gpu_ids=None):
        """
        并行训练所有 fold。
        
        Args:
            available_datasets: 用于 K 折的数据集列表
            y_trues: 测试集真实值 (MSE)
            y_trues_mae: 测试集真实值 (MAE)
            max_workers: 最大并行数
            gpu_ids: GPU ID 列表
        ...
        Returns:
            fold_results: 每折的训练结果
            fold_predictions: 每折的预测结果
        """
        k_folds = len(available_datasets)
        ...
        if max_workers is None:
            max_workers = min(k_folds, len(gpu_ids))
        
        logger.info(f"Parallel training with {max_workers} workers on GPUs: {gpu_ids}")
        
        # 准备所有 fold 的数据（转换为 numpy 以便传递给子进程）
        fold_args_list = []
        
        # 准备测试集数据（numpy 格式）
        testset_components_np = self.testset_components.cpu().numpy()
        testset_meta_np = self.testset_meta_features.cpu().numpy()
        ...
        hyperparams = {
            ...
            'k': getattr(self, 'k', 0.0),
            'temporal': getattr(self, 'temporal', 1.0),
            # ICL training variants (Fix: these were missing, causing parallel training to ignore these settings)
            'icl_shuffle': getattr(self, 'icl_shuffle', False),
            'icl_batch': getattr(self, 'icl_batch', False),
        }
        ...
        for fold_idx, val_dataset in enumerate(available_datasets):
            ...
            # 转换为 numpy
            train_data_np = {
                'components': train_data['components'].cpu().numpy(),
                'meta_features': train_data['meta_features'].cpu().numpy(),
                'targets': train_data['targets'].cpu().numpy(),
            }
            val_data_np = {
                'components': val_data['components'].cpu().numpy(),
                'meta_features': val_data['meta_features'].cpu().numpy(),
            ...
            fold_args = {
                ...
                'testset_components': testset_components_np,
                'testset_meta': testset_meta_np,
                'y_trues': y_trues,
                'y_trues_mae': y_trues_mae,
                'name_components': self.name_components,
                'gpu_id': gpu_id,
            ...
            fold_args_list.append(fold_args)
        
        # 使用线程池并行训练（对 GPU 训练更友好）
        fold_results = {}
        fold_predictions = []
        results_ordered = [None] * k_folds
        
        logger.info(f"Submitting {k_folds} fold training tasks...")
        
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 提交所有任务
            future_to_fold = {
                executor.submit(_train_fold_worker, args): args['fold_idx'] 
            ...
            for future in as_completed(future_to_fold):
                ...
                try:
                    ...
                    results_ordered[result['fold_idx']] = result
                    logger.info(f"Fold {result['fold_idx'] + 1}/{k_folds} ({result['val_dataset']}) completed: "
                               f"val_loss={result['best_val_loss']:.4f}, top1_mse={result['top1_perf']:.6f}")
                except Exception as e:
                    logger.error(f"Fold {fold_idx + 1} failed with error: {e}")
                    import traceback
                    traceback.print_exc()
                    raise
        ...
        for result in results_ordered:
            ...
            fold_predictions.append(result['y_preds'])
        
        logger.info(f"\nAll {k_folds} folds completed!")
        
        return fold_results, fold_predictions

    def meta_fit_kfold(self, fold_idx=None, parallel=False, max_workers=None, gpu_ids=None):
        """
        K折交叉验证训练。
        每个训练数据集轮流作为验证集，训练K个meta-learner，
        最后集成所有模型的预测结果。
        
        Args:
            fold_idx: 如果指定，只训练该 fold（用于手动并行训练）；否则训练所有 fold
            parallel: 是否并行训练所有 fold
            max_workers: 最大并行数（默认为 GPU 数量或 fold 数量）
            gpu_ids: 用于并行训练的 GPU ID 列表
        """
        # 如果指定了 fold_idx，只训练单个 fold
        if fold_idx is not None:
            return self.meta_fit_single_fold_only(fold_idx)
        ...
        logger.info(f"\n{'='*60}")
        logger.info(f"Starting {k_folds}-fold cross-validation" + (" (PARALLEL)" if parallel else ""))
        logger.info(f"Training datasets: {available_datasets}")
        logger.info(f"Test dataset: {self.test_dataset}")
        logger.info(f"{'='*60}\n")
        
        # 存储每折的结果
        fold_results = {}
        fold_predictions = []  # 每折模型在测试集上的预测
        
        # 获取测试集真实值（提前获取，用于计算每折的top1表现）
        y_trues = self.testset_targets.cpu().numpy()
        y_trues_mae = self.testset_targets_mae.cpu().numpy()
        has_valid_targets = not np.isnan(y_trues).all()
        ...
        # =================== 并行训练模式 ===================
        if parallel:
            fold_results, fold_predictions = self._train_folds_parallel(
                available_datasets, y_trues, y_trues_mae, max_workers, gpu_ids
            )
            
            # 并行训练后，如果需要保存 attention，在主进程中单独获取
            is_icl_model = hasattr(self, 'meta_model_type') and 'icl' in self.meta_model_type
            should_save_attention = self.save_attention and is_icl_model
            
            if should_save_attention:
                logger.info("Collecting attention scores from parallel-trained models...")
                for val_dataset, result in fold_results.items():
                    if 'model_state' in result:
                        # 准备该 fold 的训练数据
                        train_data, _ = self.prepare_fold_data(val_dataset)
                        # 重新初始化模型并获取 attention
                        self.meta_init()
                        _, attention_info = self.meta_predict_with_model(
                            result['model_state'], train_data, return_attention=True
                        )
            ...
            # =================== 顺序训练模式 ===================
            for fold_idx, val_dataset in enumerate(available_datasets):
                logger.info(f"\n--- Fold {fold_idx + 1}/{k_folds}: Validation on {val_dataset} ---")
                ...
                if should_save_attention:
                    ...
                    fold_result['attention_info'] = attention_info
                else:
                    y_preds = self.meta_predict_with_model(fold_result['model_state'], train_data)
                fold_predictions.append(y_preds)
                ...
                if has_valid_targets:
                    ...
                    fold_result['top1_perf_mae'] = fold_top1_perf_mae
                    fold_result['top1_name'] = fold_top1_name
                    logger.info(f"Fold {val_dataset}: Top1 MSE={fold_top1_perf:.6f}, Top1 MAE={fold_top1_perf_mae:.6f}, Top1={fold_top1_name}")
                else:
                    fold_result['top1_perf'] = np.nan
                    fold_result['top1_perf_mae'] = np.nan
                    fold_result['top1_name'] = self.name_components[np.argmin(y_preds)]
                
                fold_results[val_dataset] = fold_result
        ...
        fold_ranks_array = np.stack(fold_ranks)  # shape: (k_folds, n_samples)
                
        # 同时保留原始预测值的平均（用于保存和分析）
        ensemble_preds_raw = np.mean(np.stack(fold_predictions), axis=0)
        ...
        if rank_agg_method == 'mean':
            ...
            if k_folds >= 3:
                sorted_ranks = np.sort(fold_ranks_array, axis=0)
                trimmed_ranks = sorted_ranks[1:-1, :]  # 去掉最高和最低
                ensemble_ranks = np.mean(trimmed_ranks, axis=0)
            else:
                ensemble_ranks = np.mean(fold_ranks_array, axis=0)
            logger.info(f"Ensemble method: Trimmed mean (exclude min/max)")
            ...
            # 多数投票：统计进入各fold top-k的次数，次数相同时用rank中位数决胜
            vote_topk = 10  # 统计进入top-10的次数
            n_samples = fold_ranks_array.shape[1]
            vote_counts = np.zeros(n_samples)
            ...
            # 创建复合得分：主要按投票次数（取负，因为次数越多越好）
            # 次数相同时按rank中位数（越小越好）
            rank_median = np.median(fold_ranks_array, axis=0)
            # 复合得分：-vote_counts * 大数 + rank_median，这样投票次数优先
            ensemble_ranks = -vote_counts * n_samples + rank_median
            logger.info(f"Ensemble method: Voting (top-{vote_topk} counts) + median tiebreaker")
        ...
        if not has_valid_targets:
            ...
            top1_perf = y_trues[np.argmin(ensemble_preds)]
            top1_perf_mae = y_trues_mae[np.argmin(ensemble_preds)]
        ...
        # 获取top名称
        top_name = self.name_components[np.argmin(ensemble_preds)]
        topk_names = [self.name_components[i] for i in np.argsort(ensemble_preds)[:5]]
        ...
        if has_valid_targets:
            ...
            # 打印每折模型的top1表现
            logger.info(f"--- Per-Fold Top1 Performance ---")
            for val_dataset, result in fold_results.items():
                logger.info(f"  Fold {val_dataset}: Top1 MSE={result['top1_perf']:.6f}, MAE={result['top1_perf_mae']:.6f}")
        logger.info(f"{'='*60}\n")
        ...
        if self.expand_testset:
            # expand模式：选择ensemble预测的top1，运行实验
        ...
        os.makedirs(os.path.dirname(checkpoint_base_path), exist_ok=True)
        
        # 找到验证loss最低的fold作为最佳模型
        best_fold_name = min(fold_results.keys(), key=lambda x: fold_results[x]['best_val_loss'])
        best_fold_result = fold_results[best_fold_name]
        
        # 保存最佳fold的模型checkpoint
        self.save_checkpoint(
            model_state=best_fold_result['model_state'],
            save_path=f"{checkpoint_base_path}_best",
            best_epoch=best_fold_result['best_epoch'],
            val_loss=best_fold_result['best_val_loss'],
            fold_name=best_fold_name
        )
        logger.info(f"Best fold checkpoint saved (fold={best_fold_name}, val_loss={best_fold_result['best_val_loss']:.6f})")
        
        # 保存所有fold的模型checkpoint（用于ensemble inference）
        all_fold_states = {fold_name: result['model_state'] for fold_name, result in fold_results.items()}
        all_folds_checkpoint = {
            'fold_model_states': all_fold_states,
        ...
        all_folds_path = f"{checkpoint_base_path}_all_folds.pt"
        torch.save(all_folds_checkpoint, all_folds_path)
        logger.info(f"All folds checkpoint saved to: {all_folds_path}")
        
        # 保存LabelEncoders
        if hasattr(self, 'label_encoders'):
            encoders_path = f"{checkpoint_base_path}_label_encoders.pkl"
            joblib.dump(self.label_encoders, encoders_path)
            logger.info(f"LabelEncoders saved to: {encoders_path}")
        ...
        for val_dataset, result in fold_results.items():
            fold_details[val_dataset] = {
                'best_epoch': result['best_epoch'],
                'best_val_loss': result['best_val_loss'],
                'train_loss_history': result['epoch_results']['train_loss'],
                'val_loss_history': result['epoch_results']['val_loss'],
                'top1_perf': result.get('top1_perf', np.nan),
                'top1_perf_mae': result.get('top1_perf_mae', np.nan),
                'top1_name': result.get('top1_name', ''),
            }
        ...
        np.savez_compressed(
            ...
            fold_val_datasets=available_datasets,
            fold_details=fold_details,
            fold_predictions=np.stack(fold_predictions),  # 原始预测值
            fold_ranks=np.stack(fold_ranks),              # 每折的rank
            
            # 集成结果
            ensemble_ranks=ensemble_ranks,                 # rank平均值（用于选择）
            ensemble_predictions_raw=ensemble_preds_raw,   # 原始预测值平均（用于分析）
            pred_ranks_for_true_topk=pred_ranks_for_true_topk,
            true_ranks_for_pred_topk=true_ranks_for_pred_topk,
            total_num=len(ensemble_preds),
            top1_perf=top1_perf,
        ...
        if self.save_attention and hasattr(self, 'meta_model_type') and 'icl' in self.meta_model_type:
            ...
            if attention_data:
                ...
                np.savez_compressed(
                    ...
                    # Softmax Attention Scores（四个子矩阵）
                    # attention_data[fold_name] = {
                    #   'N_train', 'N_test',
                    #   'train_to_train': (num_layers, N_train, N_train) - TL block,
                    #   'train_to_test': (num_layers, N_train, N_test) - TR block (通常被 mask),
...
if __name__ == "__main__":
    ...
    # 新增：选择训练模式
    parser.add_argument('--use_kfold', type=lambda x: str(x).lower() == 'true', default=False,
                        help='If true, use K-fold cross-validation; otherwise use legacy 70/30 split')
    ...
    parser.add_argument('--parallel_folds', type=lambda x: str(x).lower() == 'true', default=False,
                        help='If true, train all folds in parallel using multiple processes.')
    parser.add_argument('--max_parallel_folds', type=int, default=None,
                        help='Maximum number of folds to train in parallel. If not set, use all available GPUs.')
    parser.add_argument('--gpu_ids', type=str, default=None,
                        help='Comma-separated GPU IDs to use for parallel training (e.g., "0,1,2,3"). '
                            'If not set, use CUDA_VISIBLE_DEVICES or all available GPUs.')
    ...
    def run_training_step():
        ...
        if args.meta_model_type == 'baseline-nn':
            logger.info("Running Baseline: Nearest Neighbor (no training, no K-fold)")
            ...
            if args.ensemble_only:
                logger.info("Running in ensemble-only mode: loading saved fold results...")
                results = meta.ensemble_from_saved_folds()
            elif args.fold_idx is not None:
                logger.info(f"Running in single-fold mode: training fold {args.fold_idx}...")
                results = meta.meta_fit_kfold(fold_idx=args.fold_idx)
            elif args.parallel_folds:
                gpu_ids = [int(x) for x in args.gpu_ids.split(',')] if args.gpu_ids else None
                logger.info(f"Running in parallel-folds mode with max_workers={args.max_parallel_folds}, gpu_ids={gpu_ids}")
                results = meta.meta_fit_kfold(
                    parallel=True, 
                    max_workers=args.max_parallel_folds,
                    gpu_ids=gpu_ids
                )
            else:
                results = meta.meta_fit_kfold()
        ...
        for idx, (pred_len_1, pred_len_2) in enumerate(pred_lens):
            ...
            if idx == 0:
                ...
                if results and 'fold_results' in results:
                    # K-fold mode: save fold results
                    trained_fold_results = results['fold_results']
                    logger.info(f"[arg_all_periods=True] Models trained (kfold), saved {len(trained_fold_results)} fold models")
