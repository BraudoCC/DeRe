...
if __name__ == '__main__':
    ...
    if 'TSGym' in setting:
        ...
        if os.path.exists(os.path.join(folder_pathGym, 'metrics.npy')):
            if not np.isnan(np.load(os.path.join(folder_pathGym, 'metrics.npy'))[1]):
    ...
    if 'Gym' in args.model:
        LOG_DB_PATH = f"{args.task_name}_TSGym_Transformer_{args.model_id.split('_')[0]}_finnal_log.db"
    else:
